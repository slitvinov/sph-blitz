...
void Boundary::BuildBoundaryParticles(ParticleManager &particles, Hydrodynamics &hydro)
{
  ...
  for(j = 1; j < y_clls - 1; j++) {
    particles.cell_lists[X][j].clear_data();
    if(xBl == 0 || xBl == 2) {
      for (LlistNode<Particle> *p10 = particles.cell_lists[Y][j].first();
	   !particles.cell_lists[Y][j].isEnd(p10);
	   p10 = particles.cell_lists[Y][j].next(p10)) {
	Particle *prtl_old = particles.cell_lists[Y][j].retrieve(p10);
	...
	particles.cell_lists[X][j].insert(particles.cell_lists[X][j].first(), prtl);
      }
    }
    if(xBl == 3) {
      for (LlistNode<Particle> *p13 = particles.cell_lists[Y][j].first();
	   !particles.cell_lists[Y][j].isEnd(p13);
	   p13 = particles.cell_lists[Y][j].next(p13)) {
	Particle *prtl_old = particles.cell_lists[Y][j].retrieve(p13);
    ...
    if(xBr == 1) {
      for (LlistNode<Particle> *p21 = particles.cell_lists[Y][j].first();
	   !particles.cell_lists[Y][j].isEnd(p21);
	   p21 = particles.cell_lists[Y][j].next(p21)) {
	Particle *prtl_old = particles.cell_lists[Y][j].retrieve(p21);
	Particle *prtl = new Particle(*prtl_old);
	Boundary_E(prtl);
	prtl->cell_i = x_clls - 1; prtl->cell_j = j;
	boundary_particle_list.insert(boundary_particle_list.first(), prtl);
  ...
  for(i = kb; i < mb; i++) {
    particles.cell_lists[i][X].clear_data();
    if(yBd == 0 || yBd == 2) {
      for (LlistNode<Particle> *p30 = particles.cell_lists[i][Y].first();
	   !particles.cell_lists[i][Y].isEnd(p30);
	   p30 = particles.cell_lists[i][Y].next(p30)) {
	Particle *prtl_old = particles.cell_lists[i][Y].retrieve(p30);
	Particle *prtl = new Particle(*prtl_old, &hydro.materials[X]);
	Boundary_S(prtl);
	prtl->cell_i = i; prtl->cell_j = 0;
	boundary_particle_list.insert(boundary_particle_list.first(), prtl);
	particles.cell_lists[i][X].insert(particles.cell_lists[i][X].first(), prtl);
      }
    }
    if(yBd == 3) {
      for (LlistNode<Particle> *p33 = particles.cell_lists[i][Y].first();
	   !particles.cell_lists[i][Y].isEnd(p33);
	   p33 = particles.cell_lists[i][Y].next(p33)) {
	Particle *prtl_old = particles.cell_lists[i][Y].retrieve(p33);
	Particle *prtl = new Particle(*prtl_old);
	Boundary_S(prtl);
	prtl->cell_i = i; prtl->cell_j = 0;
	boundary_particle_list.insert(boundary_particle_list.first(), prtl);
  ...
  for(i = ku; i < mu; i++) {
    ...
    if(yBu == 1) {
      for (LlistNode<Particle> *p41 = particles.cell_lists[i][Y].first();
	   !particles.cell_lists[i][Y].isEnd(p41);
	   p41 = particles.cell_lists[i][Y].next(p41)) {
	Particle *prtl_old = particles.cell_lists[i][Y].retrieve(p41);
	Particle *prtl = new Particle(*prtl_old);
	Boundary_N(prtl);
	prtl->cell_i = i; prtl->cell_j = y_clls - 1;
	boundary_particle_list.insert(boundary_particle_list.first(), prtl);
  ...
  if((xBl == 0 && yBd == 0) || (xBl == 2 && yBd == 2)) {
    particles.cell_lists[X][X].clear_data();
    for (LlistNode<Particle> *p130 = particles.cell_lists[Y][Y].first();
	 !particles.cell_lists[Y][Y].isEnd(p130);
	 p130 = particles.cell_lists[Y][Y].next(p130)) {
      Particle *prtl_old = particles.cell_lists[Y][Y].retrieve(p130);
      Particle *prtl = new Particle(*prtl_old, &hydro.materials[X]);
      Boundary_SW(prtl);
      prtl->cell_i = 0; prtl->cell_j = 0;
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
      particles.cell_lists[X][X].insert(particles.cell_lists[X][X].first(), prtl);
    }
  }
  if(xBl == 3 && yBd == 3) {
    particles.cell_lists[X][X].clear_data();
  ...
  if((xBl == 0 && yBu == 0) || (xBl == 2 && yBu == 2)) {
    particles.cell_lists[X][y_clls - 1].clear_data();
    for (LlistNode<Particle> *p140 = particles.cell_lists[Y][y_clls - 2].first();
	 !particles.cell_lists[Y][y_clls - 2].isEnd(p140);
	 p140 = particles.cell_lists[Y][y_clls - 2].next(p140)) {
      Particle *prtl_old = particles.cell_lists[Y][y_clls - 2].retrieve(p140);
      Particle *prtl = new Particle(*prtl_old, &hydro.materials[X]);
      Boundary_NW(prtl);
      prtl->cell_i = 0; prtl->cell_j = y_clls - 1;
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
      particles.cell_lists[X][y_clls- 1].insert(particles.cell_lists[X][y_clls - 1].first(), prtl);
    }
  }
  if(xBl == 3 && yBu == 3) {
    particles.cell_lists[X][y_clls - 1].clear_data();
  ...
  if(xBl == 1 && yBu == 1) {
    particles.cell_lists[X][y_clls - 1].clear_data();
    for (LlistNode<Particle> *p141 = particles.cell_lists[x_clls - 2][Y].first();
	 !particles.cell_lists[x_clls - 2][Y].isEnd(p141);
	 p141 = particles.cell_lists[x_clls - 2][Y].next(p141)) {
      Particle *prtl_old = particles.cell_lists[x_clls - 2][Y].retrieve(p141);
      Particle *prtl = new Particle(*prtl_old);
      Boundary_NW(prtl);
      prtl->cell_i = 0; prtl->cell_j = y_clls - 1;
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
  ...
  if(xBr == 1 && yBu == 1) {
    particles.cell_lists[x_clls - 1][y_clls - 1].clear_data();
    for (LlistNode<Particle> *p241 = particles.cell_lists[Y][Y].first();
	 !particles.cell_lists[Y][Y].isEnd(p241);
	 p241 = particles.cell_lists[Y][Y].next(p241)) {
      Particle *prtl_old = particles.cell_lists[Y][Y].retrieve(p241);
      Particle *prtl = new Particle(*prtl_old);
      Boundary_NE(prtl);
      prtl->cell_i = x_clls - 1; prtl->cell_j = y_clls - 1;
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
  ...
  if((xBr == 0 && yBd == 0) || (xBr == 2 && yBd == 2)) {
    particles.cell_lists[x_clls - 1][X].clear_data();
    for (LlistNode<Particle> *p230 = particles.cell_lists[x_clls - 2][Y].first();
	 !particles.cell_lists[x_clls - 2][Y].isEnd(p230);
	 p230 = particles.cell_lists[x_clls - 2][Y].next(p230)) {
      Particle *prtl_old = particles.cell_lists[x_clls - 2][Y].retrieve(p230);
      Particle *prtl = new Particle(*prtl_old, &hydro.materials[X]);
      Boundary_SE(prtl);
      prtl->cell_i = x_clls - 1; prtl->cell_j = 0;
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
      particles.cell_lists[x_clls - 1][X].insert(particles.cell_lists[x_clls - 1][X].first(), prtl);
    }
  }
  if(xBr == 3 && yBd == 3) {
    particles.cell_lists[x_clls - 1][X].clear_data();
  ...
  if(xBr == 1 && yBd == 1) {
    particles.cell_lists[x_clls - 1][X].clear_data();
    for (LlistNode<Particle> *p231 = particles.cell_lists[Y][y_clls - 2].first();
	 !particles.cell_lists[Y][y_clls - 2].isEnd(p231);
	 p231 = particles.cell_lists[Y][y_clls - 2].next(p231)) {
      Particle *prtl_old = particles.cell_lists[Y][y_clls - 2].retrieve(p231);
      Particle *prtl = new Particle(*prtl_old);
      Boundary_SE(prtl);
      prtl->cell_i = x_clls - 1; prtl->cell_j = 0;
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
...
void Boundary::BoundaryCondition(ParticleManager &particles)
{
  ...
  for(j = 1; j < y_clls - 1; j++) {
    if(xBl == 0 || xBl == 2) {
      for (LlistNode<Particle> *p1 = particles.cell_lists[X][j].first();
	   !particles.cell_lists[X][j].isEnd(p1);
	   p1 = particles.cell_lists[X][j].next(p1)) {
	Particle *prtl = particles.cell_lists[X][j].retrieve(p1);
  ...
  for(i = kb; i < mb; i++) {
    if(yBd == 0 || yBd == 2) {
      for (LlistNode<Particle> *p3 = particles.cell_lists[i][X].first();
	   !particles.cell_lists[i][X].isEnd(p3);
	   p3 = particles.cell_lists[i][X].next(p3)) {
	Particle *prtl = particles.cell_lists[i][X].retrieve(p3);
	prtl->StatesCopier(prtl->rl_prtl, 0);
	Boundary_S(prtl);
      }
    }
  ...
  if((xBl == 0 && yBd == 0) || (xBl == 2 && yBd == 2)) {
    for (LlistNode<Particle> *p13 = particles.cell_lists[X][X].first();
	 !particles.cell_lists[X][X].isEnd(p13);
	 p13 = particles.cell_lists[X][X].next(p13)) {
      Particle *prtl = particles.cell_lists[X][X].retrieve(p13);
      prtl->StatesCopier(prtl->rl_prtl, 0);
      Boundary_SW(prtl);
    }
  }
  ...
  if((xBl == 0 && yBu == 0) || (xBl == 2 && yBu == 2)) {
    for (LlistNode<Particle> *p14 = particles.cell_lists[X][y_clls - 1].first();
	 !particles.cell_lists[X][y_clls - 1].isEnd(p14);
	 p14 = particles.cell_lists[X][y_clls - 1].next(p14)) {
      Particle *prtl = particles.cell_lists[X][y_clls - 1].retrieve(p14);
      prtl->StatesCopier(prtl->rl_prtl, 0);
      Boundary_NW(prtl);
    }
  }
  ...
  if((xBr == 0 && yBd == 0) || (xBr == 2 && yBd == 2)) {
    for (LlistNode<Particle> *p23 = particles.cell_lists[x_clls - 1][X].first();
	 !particles.cell_lists[x_clls - 1][X].isEnd(p23);
	 p23 = particles.cell_lists[x_clls - 1][X].next(p23)) {
      Particle *prtl = particles.cell_lists[x_clls - 1][X].retrieve(p23);
      prtl->StatesCopier(prtl->rl_prtl, 0);
      Boundary_SE(prtl);
    }
  }
