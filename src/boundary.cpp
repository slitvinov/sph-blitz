...
//							constructor
//----------------------------------------------------------------------------------------
Boundary::Boundary(Initiation*   ini, 
...
//  cerr << "UyBd = " << UyBd << endl;
  ...
  BoundaryStructFactory bsf;
#ifdef DEBUG
  cout << "BSF\n";
#endif
  bsVector_.push_back(bsf.Create(W, xBl));
  bsVector_.push_back(bsf.Create(E, xBr));
  bsVector_.push_back(bsf.Create(N, yBu));
  bsVector_.push_back(bsf.Create(S, yBd));
  ...
  ini->setBoundary(this);
}
//----------------------------------------------------------------------------------------
//								Non-dimensionalize
//----------------------------------------------------------------------------------------
void Boundary::non_dimensionalize(const Initiation* const ini)
{
...
//----------------------------------------------------------------------------------------
void Boundary::show_information(const Initiation* const ini) const
{
...
void Boundary::RunAwayCheck(Hydrodynamics &hydro)
{
  ...
  for (LlistNode<Node> *p = hydro.particle_list.first(); 
    ...
    Node *prtl = hydro.particle_list.retrieve(p);
...
void Boundary::BuildBoundaryParticles(ParticleManager &particles, Hydrodynamics &hydro)
{
  ...
  for(j = 1; j < y_clls - 1; j++) {
    ...
    if(xBl == 0 || xBl == 2) {
      ...
      for (LlistNode<Node> *p10 = particles.cell_lists(1, j).first(); 
	   !particles.cell_lists(1, j).isEnd(p10); 
	   p10 = particles.cell_lists(1, j).next(p10)) {
		    
	//the original real particle
	Node *prtl_old = particles.cell_lists(1, j).retrieve(p10);
	Node *prtl = new Node(new Particle(*prtl_old, hydro.materials[0]));
    ...
    if(xBl == 3) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p13 = particles.cell_lists(1, j).first(); 
	   !particles.cell_lists(1, j).isEnd(p13); 
	   p13 = particles.cell_lists(1, j).next(p13)) {
				
	//the original real particle
	Node *prtl_old = particles.cell_lists(1, j).retrieve(p13);
	//Node *prtl = new Node(new Particle(*prtl_old));
	Node *prtl = new Node(prtl_old->createGhost());
    ...
    if(xBl == 1) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p11 = particles.cell_lists(x_clls - 2, j).first(); 
	   !particles.cell_lists(x_clls - 2, j).isEnd(p11); 
	   p11 = particles.cell_lists(x_clls - 2, j).next(p11)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(x_clls - 2, j).retrieve(p11);
	//Node *prtl = new Node(new Particle(*prtl_old));
	Node *prtl = new Node(prtl_old->createGhost());
    ...
    if(xBr == 0 || xBr == 2) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p20 = particles.cell_lists(x_clls - 2, j).first(); 
	   !particles.cell_lists(x_clls - 2, j).isEnd(p20); 
	   p20 = particles.cell_lists(x_clls - 2, j).next(p20)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(x_clls - 2, j).retrieve(p20);
	Node *prtl = new Node(new Particle(*prtl_old, hydro.materials[0]));
	...
	boundary_particle_list.insert(boundary_particle_list.first(), prtl);
	//insert the position into corresponding cell list
	//	cerr << "The particle must be inserted here" << endl;
	particles.cell_lists(x_clls- 1, j).insert(particles.cell_lists(x_clls - 1, j).first(),
						  prtl);
      }
    }
    ...
    if(xBr == 3) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p23 = particles.cell_lists(x_clls - 2, j).first(); 
	   !particles.cell_lists(x_clls - 2, j).isEnd(p23); 
	   p23 = particles.cell_lists(x_clls - 2, j).next(p23)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(x_clls - 2, j).retrieve(p23);
	//Node *prtl = new Node(new Particle(*prtl_old));
	Node *prtl = new Node(prtl_old->createGhost());
    ...
    if(xBr == 1) {
      ...
      for (LlistNode<Node> *p21 = particles.cell_lists(1, j).first(); 
	   !particles.cell_lists(1, j).isEnd(p21); 
	   p21 = particles.cell_lists(1, j).next(p21)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(1, j).retrieve(p21);
	//Node *prtl = new Node(new Particle(*prtl_old));
	Node *prtl = new Node(prtl_old->createGhost());
  ...
  for(i = kb; i < mb; i++) {
    ...
    if(yBd == 0 || yBd == 2) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p30 = particles.cell_lists(i, 1).first(); 
	   !particles.cell_lists(i, 1).isEnd(p30); 
	   p30 = particles.cell_lists(i, 1).next(p30)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(i, 1).retrieve(p30);
	Node *prtl = new Node(new Particle(*prtl_old, hydro.materials[0]));
    ...
    if(yBd == 3) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p33 = particles.cell_lists(i, 1).first(); 
	   !particles.cell_lists(i, 1).isEnd(p33); 
	   p33 = particles.cell_lists(i, 1).next(p33)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(i, 1).retrieve(p33);
	//Node *prtl = new Node(new Particle(*prtl_old));
	Node *prtl = new Node(prtl_old->createGhost());
    ...
    if(yBd == 1) {
      //iterate the correspeond cell for real and wall partilces
      for (LlistNode<Node> *p31 = particles.cell_lists(i, y_clls - 2).first(); 
	   !particles.cell_lists(i, y_clls - 2).isEnd(p31); 
	   p31 = particles.cell_lists(i, y_clls - 2).next(p31)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(i, y_clls - 2).retrieve(p31);
	//Node *prtl = new Node(new Particle(*prtl_old));
	Node *prtl = new Node(prtl_old->createGhost());
  ...
  for(i = ku; i < mu; i++) {
    ...
    if(yBu == 0 || yBu == 2) {
      //iterate the correspeond cell for real and wall partilces
      for (LlistNode<Node> *p40 = particles.cell_lists(i, y_clls - 2).first(); 
	   !particles.cell_lists(i, y_clls - 2).isEnd(p40); 
	   p40 = particles.cell_lists(i, y_clls - 2).next(p40)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(i, y_clls - 2).retrieve(p40);
	Node *prtl = new Node(new Particle(*prtl_old, hydro.materials[0]));
    ...
    if(yBu == 3) {
      //iterate the correspeond cell for real and wall partilces
      for (LlistNode<Node> *p43 = particles.cell_lists(i, y_clls - 2).first(); 
	   !particles.cell_lists(i, y_clls - 2).isEnd(p43); 
	   p43 = particles.cell_lists(i, y_clls - 2).next(p43)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(i, y_clls - 2).retrieve(p43);
	//Node *prtl = new Node (new Particle(*prtl_old));
	Node *prtl = new Node(prtl_old->createGhost());
    ...
    if(yBu == 1) {
      //iterate the correspeond cell for real and wall partilces
      for (LlistNode<Node> *p41 = particles.cell_lists(i, 1).first(); 
	   !particles.cell_lists(i, 1).isEnd(p41); 
	   p41 = particles.cell_lists(i, 1).next(p41)) {
					
	//the original real particle
	Node *prtl_old = particles.cell_lists(i, 1).retrieve(p41);
	//Node *prtl = new Node(new Particle(*prtl_old));
	Node *prtl = new Node(prtl_old->createGhost());
  ...
  if(xBl == 0 && yBd == 0 || xBl == 2 && yBd == 2) {
    ...
    //iterate the correspeond cell linked list
    for (LlistNode<Node> *p130 = particles.cell_lists(1, 1).first(); 
	 !particles.cell_lists(1, 1).isEnd(p130); 
	 p130 = particles.cell_lists(1, 1).next(p130)) {
					
      //the original real particle
      Node *prtl_old = particles.cell_lists(1, 1).retrieve(p130);
      Node *prtl = new Node(new Particle(*prtl_old, hydro.materials[0]));
  ...
  if(xBl == 1 && yBd == 1) {
    ...
    particles.cell_lists(0, 0).clear_data();
    //iterate the correspeond cell for real and wall partilces
    for (LlistNode<Node> *p131 = particles.cell_lists(x_clls - 2, y_clls - 2).first(); 
	 !particles.cell_lists(x_clls - 2, y_clls - 2).isEnd(p131); 
	 p131 = particles.cell_lists(x_clls - 2, y_clls - 2).next(p131)) {
					
      //the original real particle
      Node *prtl_old = particles.cell_lists(x_clls - 2, y_clls - 2).retrieve(p131);
      //Node *prtl =new Node( new Particle(*prtl_old));
      Node *prtl = new Node(prtl_old->createGhost());
  ...
  if(xBl == 0 && yBu == 0 || xBl == 2 && yBu == 2) {
    ...
    //iterate the correspeond cell for real and wall partilces
    for (LlistNode<Node> *p140 = particles.cell_lists(1, y_clls - 2).first(); 
	 !particles.cell_lists(1, y_clls - 2).isEnd(p140); 
	 p140 = particles.cell_lists(1, y_clls - 2).next(p140)) {
				
      //the original real particle
      Node *prtl_old = particles.cell_lists(1, y_clls - 2).retrieve(p140);
      Node *prtl = new Node(new Particle(*prtl_old, hydro.materials[0]));
      ...
      //			prtl->setCellPosition(0, y_clls - 1);
      //insert its poistion on the particle list
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
      //insert the position into corresponding cell list
  ...
  if(xBl == 1 && yBu == 1) {
    ...
    particles.cell_lists(0, y_clls - 1).clear_data();
    //iterate the correspeond cell for real and wall partilces
    for (LlistNode<Node> *p141 = particles.cell_lists(x_clls - 2, 1).first(); 
	 !particles.cell_lists(x_clls - 2, 1).isEnd(p141); 
	 p141 = particles.cell_lists(x_clls - 2, 1).next(p141)) {
				
      //the original real particle
      Node *prtl_old = particles.cell_lists(x_clls - 2, 1).retrieve(p141);
      //Node *prtl = new Node(new Particle(*prtl_old));
      Node *prtl = new Node(prtl_old->createGhost());
  ...
  if(xBr == 0 && yBu == 0 || xBr == 2 && yBu == 2) {
    ...
    //iterate the correspeond cell linked list
    for (LlistNode<Node> *p240 = particles.cell_lists(x_clls - 2, y_clls - 2).first(); 
	 !particles.cell_lists(x_clls - 2, y_clls - 2).isEnd(p240); 
	 p240 = particles.cell_lists(x_clls - 2, y_clls - 2).next(p240)) {
				
      //the original real particle
      Node *prtl_old = particles.cell_lists(x_clls - 2, y_clls - 2).retrieve(p240);
      Node *prtl = new Node(new Particle(*prtl_old, hydro.materials[0]));
      ...
      //			prtl->setCellPosition(x_clls - 1, y_clls - 1);
      //insert its poistion on the particle list
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
      //insert the position into corresponding cell list
  ...
  if(xBr == 1 && yBu == 1) {
    ...
    particles.cell_lists(x_clls - 1, y_clls - 1).clear_data();
    //iterate the correspeond cell for real and wall partilces
    for (LlistNode<Node> *p241 = particles.cell_lists(1, 1).first(); 
	 !particles.cell_lists(1, 1).isEnd(p241); 
	 p241 = particles.cell_lists(1, 1).next(p241)) {
					
      //the original real particle
      Node *prtl_old = particles.cell_lists(1, 1).retrieve(p241);
      //Node *prtl = new Node (new Particle(*prtl_old));
      Node *prtl = new Node(prtl_old->createGhost());
  ...
  if(xBr == 0 && yBd == 0 || xBr == 2 && yBd == 2) {
    ...
    //iterate the correspeond cell linked list
    for (LlistNode<Node> *p230 = particles.cell_lists(x_clls - 2, 1).first(); 
	 !particles.cell_lists(x_clls - 2, 1).isEnd(p230); 
	 p230 = particles.cell_lists(x_clls - 2, 1).next(p230)) {
				
      //the original real particle
      Node *prtl_old = particles.cell_lists(x_clls - 2, 1).retrieve(p230);
      Node *prtl = new Node (new Particle(*prtl_old, hydro.materials[0]));
      ...
      //			prtl->setCellPosition(x_clls - 1, 0);
      //insert its poistion on the image particle list
      boundary_particle_list.insert(boundary_particle_list.first(), prtl);
      //insert the position into corresponding cell list
  ...
  if(xBr == 1 && yBd == 1) {
    ...
    particles.cell_lists(x_clls - 1, 0).clear_data();
    //iterate the correspeond cell for real and wall partilces
    for (LlistNode<Node> *p231 = particles.cell_lists(1, y_clls - 2).first(); 
	 !particles.cell_lists(1, y_clls - 2).isEnd(p231); 
	 p231 = particles.cell_lists(1, y_clls - 2).next(p231)) {
					
      //the original real particle
      Node *prtl_old = particles.cell_lists(1, y_clls - 2).retrieve(p231);
      //Node *prtl = new Node (new Particle(*prtl_old));
      Node *prtl = new Node(prtl_old->createGhost());
...
//   cerr << "==Boundary check==" << endl;
//   for (int i=0; i<x_clls; ++i) {
//     for (int j=0; j<y_clls; ++j) {
//       cerr << "cell_list[" << i << "][" << j << "] = " << particles.cell_lists(i, j).length() << endl;
//     }
//   }
}
//----------------------------------------------------------------------------------------
...
void Boundary::BoundaryCondition(ParticleManager &particles)
{
  ...
  for(j = 1; j < y_clls - 1; j++) {
    ...
    if(xBl == 0 || xBl == 2) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p1 = particles.cell_lists(0, j).first(); 
	   !particles.cell_lists(0, j).isEnd(p1); 
	   p1 = particles.cell_lists(0, j).next(p1)) {
				
	Node *prtl = particles.cell_lists(0, j).retrieve(p1);
    ...
    if(xBr == 0 || xBr == 2) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p2 = particles.cell_lists(x_clls - 1, j).first(); 
	   !particles.cell_lists(x_clls - 1, j).isEnd(p2); 
	   p2 = particles.cell_lists(x_clls - 1, j).next(p2)) {
					
	Node *prtl = particles.cell_lists(x_clls - 1, j).retrieve(p2);
	//copy states from the original particle
  ...
  for(i = kb; i < mb; i++) {
    ...
    if(yBd == 0 || yBd == 2) {
      //iterate the correspeond cell linked list
      for (LlistNode<Node> *p3 = particles.cell_lists(i, 0).first(); 
	   !particles.cell_lists(i, 0).isEnd(p3); 
	   p3 = particles.cell_lists(i, 0).next(p3)) {
					
	Node *prtl = particles.cell_lists(i, 0).retrieve(p3);
	//copy states from the original particle
	//prtl->StatesCopier(*prtl->rl_prtl, 0);
	prtl->StatesCopier(0);
  ...
  for(i = ku; i < mu; i++) {
    ...
    if(yBu == 0 || yBu == 2) {
      //iterate the correspeond cell for real and wall partilces
      for (LlistNode<Node> *p4 = particles.cell_lists(i, y_clls - 1).first(); 
	   !particles.cell_lists(i, y_clls - 1).isEnd(p4); 
	   p4 = particles.cell_lists(i, y_clls - 1).next(p4)) {
					
	//the original real particle
	Node *prtl = particles.cell_lists(i, y_clls - 1).retrieve(p4);
	//copy states from the original particle
	//prtl->StatesCopier(*prtl->rl_prtl, 0);
	prtl->StatesCopier(0);
  ...
  if(xBl == 0 && yBd == 0 || xBl == 2 && yBd == 2) {
    //iterate the correspeond cell linked list
    for (LlistNode<Node> *p13 = particles.cell_lists(0, 0).first(); 
	 !particles.cell_lists(0, 0).isEnd(p13); 
	 p13 = particles.cell_lists(0, 0).next(p13)) {
					
      Node *prtl = particles.cell_lists(0, 0).retrieve(p13);
      //copy states from the original particle
      //prtl->StatesCopier(*prtl->rl_prtl, 0);	
      prtl->StatesCopier(0);
  ...
  if(xBl == 0 && yBu == 0 || xBl == 2 && yBu == 2) {
    //iterate the correspeond cell for real and wall partilces
    for (LlistNode<Node> *p14 = particles.cell_lists(0, y_clls - 1).first(); 
	 !particles.cell_lists(0, y_clls - 1).isEnd(p14); 
	 p14 = particles.cell_lists(0, y_clls - 1).next(p14)) {
					
      //the original real particle
      Node *prtl = particles.cell_lists(0, y_clls - 1).retrieve(p14);
      //copy states from the original particle
      //prtl->StatesCopier(*prtl->rl_prtl, 0);
      prtl->StatesCopier(0);
  ...
  if(xBr == 0 && yBu == 0 || xBr == 2 && yBu == 2) {
    //iterate the correspeond cell linked list
    for (LlistNode<Node> *p24 = particles.cell_lists(x_clls - 1, y_clls - 1).first(); 
	 !particles.cell_lists(x_clls - 1, y_clls - 1).isEnd(p24); 
	 p24 = particles.cell_lists(x_clls - 1, y_clls - 1).next(p24)) {
				
      //the original real particle
      Node *prtl = particles.cell_lists(x_clls - 1, y_clls - 1).retrieve(p24);
      //copy states from the original particle
      //prtl->StatesCopier(*prtl->rl_prtl, 0);
      prtl->StatesCopier(0);
  ...
  if(xBr == 0 && yBd == 0 || xBr == 2 && yBd == 2) {	
    //iterate the correspeond cell linked list
    for (LlistNode<Node> *p23 = particles.cell_lists(x_clls - 1, 0).first(); 
	 !particles.cell_lists(x_clls - 1, 0).isEnd(p23); 
	 p23 = particles.cell_lists(x_clls - 1, 0).next(p23)) {
					
      //the original real particle
      Node *prtl = particles.cell_lists(x_clls - 1, 0).retrieve(p23);
      //copy states from the original particle
      //prtl->StatesCopier(*prtl->rl_prtl, 0);
      prtl->StatesCopier(0);
  ...
  //  std::cerr << "The end of boundary lists cleations" << std::endl;
}
//----------------------------------------------------------------------------------------
//			implement west side boubadry by modeify particle states
//----------------------------------------------------------------------------------------
void Boundary::Boundary_W(Node *prtl)
{
  //the rigid wall conditions	
  switch(xBl) {
    //wall
...
//----------------------------------------------------------------------------------------
void Boundary::Boundary_E(Node *prtl)
{
  //the rigid wall conditions	
  switch(xBr) {
    //wall
...
//----------------------------------------------------------------------------------------
void Boundary::Boundary_S(Node *prtl)
{
  //the rigid wall conditions	
  switch(yBd) {
    //wall
...
//----------------------------------------------------------------------------------------
void Boundary::Boundary_N(Node *prtl)
{
  //the rigid wall conditions	
  switch(yBu) {
    //wall
...
//----------------------------------------------------------------------------------------
void Boundary::Boundary_SW(Node *prtl)
{
  //the rigid wall conditions	
  switch(xBl) {
    //wall
...
//----------------------------------------------------------------------------------------
void Boundary::Boundary_NW(Node *prtl)
{
	//the rigid wall conditions	
	switch(xBl) {
	//wall
...
//----------------------------------------------------------------------------------------
void Boundary::Boundary_NE(Node *prtl)
{
	//the rigid wall conditions	
	switch(xBr) {
	//wall
...
//			implement north-east corner boubadry by modeify particle states
//----------------------------------------------------------------------------------------
void Boundary::Boundary_SE(Node *prtl)
{
	//the rigid wall conditions	
	switch(xBr) {
	//wall
...
Vec2d Boundary::getBDistance(const Vec2d& x, const Vec2d& y) const {
#ifndef NOLOG
  log4cxx::LoggerPtr Logger = log4cxx::Logger::getLogger("boundary");
  LOG4CXX_DEBUG(Logger, "getBDistance");
#endif  
  Vec2d vec = y - x;
  double mind = dot(vec, vec);
  for (unsigned int q=0; q<bsVector_.size(); q++) {
#ifndef NOLOG
    LOG4CXX_DEBUG(Logger, "BoundaryStruct number: " + stringify<unsigned int>(q));
#endif
    Vec2d cvec = bsVector_.at(q)->getDistance(x, y);    
    double cmin = dot(cvec, cvec);
    if (cmin<mind) {
      mind = cmin;
      vec = cvec;
    }
