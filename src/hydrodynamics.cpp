  ...
  while (fscanf (f, "%s", Key_word) == 1)
	...
	for (k = 0; k < number_of_materials; k++)
	  {
	    mtl = &materials[k];
	    mtl->number = k;
	    if (fscanf (f, "%s %d", mtl->material_name, &mtl->material_type)
		!= 2)
	      ABORT (("can't read material from '%s'", ini->inputfile));
	    if (fscanf (f, "%lf %lf %lf %lf %lf",
			&mtl->eta, &mtl->zeta, &mtl->gamma, &mtl->rho0,
			&mtl->a0) != 5)
	...
	for (l = 0; l < number_of_materials; l++)
	  for (n = 0; n < number_of_materials; n++)
		...
		ABORT (("can't read materal from '%s'", ini->inputfile));
	      force = &forces[k][m];
	      if (fscanf (f, "%lf %lf %lf %lf",
			  &force->epsilon, &force->sigma,
			  &force->shear_slip, &force->bulk_slip) != 4)
		ABORT (("can't read force from '%s'", ini->inputfile));
	    }
    }
  fclose (f);
  viscosity_max = 0.0;
