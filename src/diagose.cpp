...
Diagnose::Diagnose(Initiation &ini, Hydrodynamics &hydro)
{
	...
	/// <ul><li> copy parameters from Initiation class
	x_cells = ini.x_cells; y_cells = ini.y_cells;
	hdelta = ini.hdelta; 
	delta = ini.delta;

	/// <li>copy boundary properties from initiation class
	...
	if(ini.diagnose == 2) {
		...
		char file_name[50];
		/// <li> produce output file name ("kinetic_info.dat")and file header
	...
	cout<<"\n Diagnose class object successfully initiated \n";
}
//----------------------------------------------------------------------------------------
...
void Diagnose::SaveStates(Hydrodynamics &hydro)
{
	...
	///- Insert the states of the particles in the states vectores (vx_list, vy_list, rho_list)
...
void Diagnose::OutputProfile(double Time, Initiation &ini)
{
	...
	///- produce output file name "dstrXXXX.dat"
	...
	ofstream out(file_name);
	///- defining file header for tecplot(plot software)
...
void Diagnose::OutputAverage(double Time, Initiation &ini)
{
	...
	///- loop the grid points and output data
	//NOTE: loop the x direction first and then the y direction!
	for(j = 0; j < gridy; j++) { 
		for(i = 0; i < gridx; i++) {
			pstn[0] = i*delta; pstn[1] = j*delta;
