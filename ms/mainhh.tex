...
give exact results, as the integral, respectively the sum, is truncated due to
the simple fact that outside the domain there are no
particles~\cite{Liu2003}. 
%(find better citation...) 
This effect is called the {\bf particle deficiency
problem}.\cite{Liu2003}. In cases where the region of interest is far from the edge of the domain and where erroneous information from the edges do not propagate to the region of interest during the time in consideration, there is no need to take this
problem into account. 
The 1D shock-tube simulation described in sections (\ref{sec:1DSPHcodeResults}) and (\ref{sec:2DSPHcodeResults}) is an example of such a case.  
But for other applications the edges play an important role, or the calculation domain has to be continued periodically. In these cases it is necessary to define specific boundary
conditions. This can be accomplished by a special boundary treatment which
often includes so--called virtual particles or {\bf ghost
...
\begin{equation}
{\bf v_{virtual}}=2{\bf v_{wall}}-{\bf v_{real}}
\end{equation}

Free slip velocity boundary condition:

\begin{equation}
{\bf v_{virtual}^{tangential}}={\bf v_{real}^{tangential}}
\end{equation}
...
%obige aussage Falsch, schließlich müssen partikel trotzdem kopiert werden, um als nachbar-particle für interpolation zur verfügung zu stehen

%außerdem muss in diesen Abschnitt
% A) die Geschichte mit linearer, quadratischer iterpolation besser als Spiegelung der GEschwidnigkeit für viskose Randschichten in der Anfangsphase (steht irgendwo in meiner README)
% B) die Randedingungen für die Wärmeleitung, sofern noch andere implementiert als periodisch und symmetrisch(=adiabat), beide funktionieren ja automatisch
...
Having a look at figure (\ref{fig:2DSPHresults_1D_CS_dx_const}), one sees the above anticipated tendency. Especially the integral errors ($L_1,L_2$) calculated for the whole domain show a clear minimum at a supportlength of around 0.025. 
This means that the number of neighnouring particles is 8. 
...
Despite of this change in properties in the second direction, the final results for the qD shock--tube problem, i.e. the averaged values, are very accurate and totally comparable to the corresponding simulation simulation with purely 1D particle distribution. Sometimes the averaged 2D results are even more accurate.
...
\caption[Convergence Shock-tube with 1D constant mass particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with a varying supoprt length and a constant particle spacing mass of $m=0.001875$ leading to a spacing on the LHS of $dx_l=0.001875$ and $dx_r=0.015$ on the RHS}
...
\caption[Convergence Shock-tube 1D constant spacing particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing resolution and a constant ratio of support length to spacing of 5 ($h/m=8$)}
...
\caption[Convergence Shock-tube with 2D constant spacing particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing resolution and a constant ratio of support length to spacing of 5 ($h/m=8$)}
...
\caption[Convergence Shock-tube 1D constant mass particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing resolution and a constant ratio of support length to mass of 16 ($h/m=8$)}
