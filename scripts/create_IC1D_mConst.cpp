...
int main ()
{
 ...
 ofstream out("../cases/1DshockMConst.ivs");

 out<<N<<endl;

for(int i=0;i<N;i++)
   {
     out <<setprecision (8)<<"  "<<x_array[i]<<"  "<<y_array[i]<<"   "<<vx_array[i]<<"   "<<vy_array[i]<<"   "<<rho_array[i]<<"   "<<p_array[i]<<"  "<<m<<endl;
   };

 out.close();
 cout<<"1DshockMConst.ivs successfully written in <<sph-blitz/cases>>\n";
}
