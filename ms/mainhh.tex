...
\usepackage[a4paper,left=2.4cm,right=2.4cm]{geometry}
\usepackage{appendix}%for additional appendix options
%\usepackage{float} %when using [H]: figure placed at exact spot
\usepackage{afterpage,flafter,float}  %TANJA: Umgang mit Gleitobjekten
\usepackage[latin1]{inputenc} %Umlaute
\usepackage{graphicx}
\usepackage{verbatim} % for multi-line comments
% \usepackage{listings} %von Tanja
\usepackage{listings,fancyvrb} %von Tanja
\usepackage{amsmath}
...
\usepackage{fancyhdr}  % TANJA:Kopf/Fu"szeilen

%\usepackage{fullpage}
\usepackage{dcolumn} % TANJA: Zahlen in Tab auf Dez. Punkt ausrichten
...
%\addtolength{\hoffset}{-1.3cm} %Tanja: hab ich in Kommentar setze
%\addtolength{\textwidth}{3.5cm} %Tanja: hab ich in Kommentar setze
...
%\usepackage[pdftex]{color}		% aus anderer Vorlage

%Farben aus dem Corporate design der TUM
%\definecolor{p300}{cmyk}{1,0.43,0,0}
%\definecolor{light}{cmyk}{0,0,0,0.3}
%\definecolor{heavy}{cmyk}{0,0,0,0.6}


\renewcommand{\baselinestretch}{1.1}   % Zeilenabstand
\renewcommand{\arraystretch}{1.4}      % Zeilenabstand in Tabellen
%\renewcommand{\familydefault}{cmss}    % Serifenlose Schrift --> wenn ich das in Kommentar setze, ist der Haupttext in Times New Roman...


\newcommand{\clearemptydoublepage}{{\cleardoublepage\pagestyle{empty}}}


\lhead[\fancyplain{}{{\includegraphics[height=12mm]{fig/caltech_logo}\hspace{5mm}\rightmark}}]%
       {\fancyplain{}{}}
 \rhead[\fancyplain{}{}]%
       {\fancyplain{}{{\leftmark\hspace{5mm}\includegraphics[height=12mm]{fig/TUMLogo_oZ_Vollfl_blau_RGB1}}}}
\cfoot{}
\rfoot[{}]
       {{Page \thepage}}
\lfoot[{{Page \thepage}}]%
...
% Mehrzeilige, zentrierte Tabellenspalten

 \newcommand{\PreserveBackslash}[1]{\let\temp=\\#1\let\\=\temp}
\let\PSB=\PreserveBackslash
%\PSB=PreserveBackslash (LaTeX Begleiter S. 110)
\newcolumntype{C}[1]{>{\PSB\centering\hspace{0pt}}m{#1}}
\newcolumntype{Z}[1]{>{\PSB\centering\hspace{0pt}$}m{#1}<{$}}
...
\chapter*{Abstract}
%One Page abstraction of the thesis in \bfseries{German AND
% English}\mdseries
...
\include{Symbols} %in diese datei symbole rein (table style)
\cleardoublepage
...
The particle manager is in charge of the particles at a higher level. There is for example a method generating all particles as initialized via the (.ivs) file. For this purpose the (.ivs) file is read and each particle generated by the corresponding constructor of the {\tt Particle} class. Depending on their types, particles are then assigned to the corresponding lists.
Furthermore the {\tt cell\_lists} Matrix is managed by a particle manager method. This method updates all list entries according to the current particle positions

\paragraph{The {\tt Hydrodynamics} class}
The hydrodynamics class contains various methods at a high or intermediate level. One exemplary method generates all interaction pairs, others update the change rates for all particles and again others update the particles values based on these change rates. A further method of hydrodynamics determines the automatic time step according to section (\ref{sec:TmeStepChoice}). 

\paragraph{The {\tt Output} class}
After a user selected output interval is reached, the output method writes the current values into a file {\tt prtlxxxxxxxx.dat} in the folder {\tt src/outdata}, where xxxxxxxx is the time stamp. A restart (.rst) file is produced as well in the {\tt cases} folder. If the restart option is selected in the setup (.tcl) file, the simulation can then by resumed from the previously reached instant.

\paragraph{The {\tt TimeSolver} class}
Numerical integration schemes are implemented in the {\tt TimeSolver} class, which again is an abstract base class with the following implementations.
\begin{itemize}
 \item {\tt GasTimeSolverLeapFrog}: Time integration is conducted with the leap--frog scheme introduced in section (\ref{sec:numIntegr}). This time solver is exclusively used for the gas dynamics mode.
\item {\tt GasTimeSolverPredCorr}: Time integration is performed using a predictor corrector scheme as presented in section (\ref{sec:numIntegr}). This time solver is only used for the gas dynamics mode.
\item {\tt HydroTimeSolver}: The hydro dynamics mode time solver is a remainder of the original code.
\end{itemize}
...
\begin{tabular}[c]{|l|p{5cm}|} % die eckige Klammer steht f�r die Ausrichtung der Tabelle im Text: c steht f�r zentriert
\hline
\hline
domain size &  2\\
\hline
...
The shock--tube case is used to conduct several series of simulations aimed at the verification of the modified code and at assessing %its convergence behavior %(IST DAS NICHT ABHÄNGIG VOM TESTFALL??) as well as 
...
\caption[Convergence Shock-tube 1D constant spacing particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with a varying time step $dt$. The support length is $0.03$ with an 1D particle distribution of constant spacing of $dx=0.005$. Explanation on the meaning of the different areas is given in section (\ref{sec:2Dshock_simuSetup&Co})}
...
\caption[Convergence Shock-tube 1D constant spacing particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with a varying support length and a constant particle spacing of $dx=0.0025$. Explanation on the meaning of the different areas is given in section (\ref{sec:2Dshock_simuSetup&Co})}
...
\subfigure[Perturbed initial particle field]{
\label{fig:2DSPHresults_perturbationComparisonProfiles_perturbed}
\includegraphics[width=7cm]{Graphics/results/ShockTube/PertStudy/comparisonEnergyProfiles/pert/E_profiles00199793}
}
\subfigure[Unperturbed initial particle field]{
\label{fig:2DSPHresults_perturbationComparisonProfiles_UNperturbed}
\includegraphics[width=7cm]{Graphics/results/ShockTube/PertStudy/comparisonEnergyProfiles/unpert/E_profiles00200908}
}
...
\caption[Perturbation influence shock-tube with 1D constant spacing particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing perturbation of initial particle position based on a 1D constant spacing configuration. The employed support length was $l_s=0.025$ and the spacing $dx=0.005$. An explanation on the meaning of the different areas is given in section (\ref{sec:2Dshock_simuSetup&Co}).}
...
\caption[Perturbation influence shock-tube with 1D constant mass particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing perturbation of initial particle position based on a 1D constant mass configuration with a support length of $l_s=0.025$ and a particle mass of $1.5625\cdot10^{-3}$. Explanation on the meaning of the different areas is given in section (\ref{sec:2Dshock_simuSetup&Co}).}
...
\includegraphics[width=7cm]{Graphics/results/Porosities/LinearWall/Poiseuille_DomainEdgeBC_N4x40_suplen2dx_t3sec/Energy}
}
\subfigure[density]{
\label{fig:Porosities_LinearWall_domainEdgeBC_tDensity}
\includegraphics[width=7cm]{Graphics/results/Porosities/LinearWall/Poiseuille_DomainEdgeBC_N4x40_suplen2dx_t3sec/Density}
}
...
There is generally a considerable relative difference in these quantities for the row of particles closest to the wall %(I IMPLICITELY assume that Method 1 gives the (more) accurate density results)). 
...
\includegraphics[height=7cm]{Graphics/results/Porosities/Cavity/supLen_dx_4_dx0_2_SumDens_g0_2/u25mod}
}
\subfigure[Density field]{
\label{fig:Cavities_steadyState_rho}
\includegraphics[height=7cm]{Graphics/results/Porosities/Cavity/supLen_dx_4_dx0_2_SumDens_g0_2/rho25mod}
}
\subfigure[Pressure field]{
\label{fig:Cavities_steadyState_p}
\includegraphics[height=7cm]{Graphics/results/Porosities/Cavity/supLen_dx_4_dx0_2_SumDens_g0_2/p25mod}
}
\subfigure[Energy field]{
\label{fig:Cavities_steadyState_e}
\includegraphics[height=7cm]{Graphics/results/Porosities/Cavity/supLen_dx_4_dx0_2_SumDens_g0_2/e25mod}
}
...
\includegraphics[width=7cm]{Graphics/results/Porosities/Cylinders/supLen_dx_4_dx0_2_hexCyl_SumDens/steady_u_inclArrows_mod}
}
\subfigure[Density field]{
\label{fig:Cylinders_Hex_steadyState_rho}
\includegraphics[width=7cm]{Graphics/results/Porosities/Cylinders/supLen_dx_4_dx0_2_hexCyl_SumDens/steady_rho_mod}
}
\subfigure[Pressure field]{
\label{fig:Cylinders_Hex_steadyState_p}
\includegraphics[width=7cm]{Graphics/results/Porosities/Cylinders/supLen_dx_4_dx0_2_hexCyl_SumDens/steady_p_mod}
}
\subfigure[Energy field]{
\label{fig:Cylinders_Hex_steadyState_e}
\includegraphics[width=7cm]{Graphics/results/Porosities/Cylinders/supLen_dx_4_dx0_2_hexCyl_SumDens/steady_e_mod}
}
...
%, i.\ e.\ how values can be transferreed from one 
...
\label{app:additionalShockResults}
\begin{figure}[H]
\centering
...
\caption[Convergence Shock-tube with 2D constant spacing particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulations with a varying support length and a constant initial spacing 2D particle field with $dx=dy=0.005$.}
...
\caption[Convergence Shock-tube with 2D constant mass particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with a varying support length and a constant particle mass of $m=2.5\cdot10^{-5}$. This leads to a particle spacing of $dx_l=dy_l=0.005$ on the LHS and of $dx_r=dy_r=0.015$ on the RHS of the 2D particle field.}
...
\caption[Convergence Shock-tube with 2D constant spacing particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing resolution and a constant ratio of support length to spacing of 5 evaluated at $t=0.2$. The initial particle field is a 1D constant spacing configuration.}
...
\caption[Convergence Shock-tube 1D constant mass particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing resolution and a constant ratio of support length to mass of $l_s/m=16$. The initial particle field is a 1D constant mass configuration. }

\end{figure}

\begin{figure}[H]
\centering
\label{fig:2DSPHresults_q1D_CM_ratioConst}

\subfigure[tight][whole domain]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_wholeRho}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/WholeDomainRho}
}
\subfigure[tight][whole domain]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_3Rho}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/Area3Rho}
}
\subfigure[tight][whole domain]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_2Rho}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/Area2Rho}
}
\subfigure[tight][area 3]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_wholeU}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/WholeDomainU}
}
\subfigure[tight][area 3]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_3U}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/Area3U}
}
\subfigure[tight][area 3]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_2U}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/Area2U}
}
\subfigure[tight][area 2]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_wholeE}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/WholeDomainE}
}
\subfigure[tight][area 2]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_3E}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/Area3E}
}
\subfigure[tight][area 2]{
\label{fig:2DSPHresults_q1D_CM_ratioConst_2E}
\includegraphics[width=5cm]{Graphics/results/ShockTube/q1D_CM_LF_SD/ratioSupLen_sqrt_m6/Area2E}
}

\caption[Convergence Shock-tube with 2D constant mass particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing resolution and a constant ratio of $l_s/\sqrt{m}=6$. The simulation is evaluated at $t=0.2$. The initial particle field is a 2D constant mass configuration. }
...
\caption[Perturbation influence shock-tube with 2D constant spacing particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing perturbation of initial particle position based on a 2D constant spacing particle field.}

\end{figure}

\begin{figure}[H]
\centering
\label{fig:2DSPHresults_2D_CM_pertStudy}

\subfigure[tight][whole domain]{
\label{fig:2DSPHresults_2D_CM_pertStudy_wholeRho}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/WholeDomainRho}
}
\subfigure[tight][whole domain]{
\label{fig:2DSPHresults_2D_CM_pertStudy_3Rho}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/Area3Rho}
}
\subfigure[tight][whole domain]{
\label{fig:2DSPHresults_2D_CM_pertStudy_2Rho}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/Area2Rho}
}
\subfigure[tight][area 3]{
\label{fig:2DSPHresults_2D_CM_pertStudy_wholeU}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/WholeDomainU}
}
\subfigure[tight][area 3]{
\label{fig:2DSPHresults_2D_CM_pertStudy_3U}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/Area3U}
}
\subfigure[tight][area 3]{
\label{fig:2DSPHresults_2D_CM_pertStudy_2U}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/Area2U}
}
\subfigure[tight][area 2]{
\label{fig:2DSPHresults_2D_CM_pertStudy_wholeE}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/WholeDomainE}
}
\subfigure[tight][area 2]{
\label{fig:2DSPHresults_2D_CM_pertStudy_3E}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/Area3E}
}
\subfigure[tight][area 2]{
\label{fig:2DSPHresults_2D_CM_pertStudy_2E}
\includegraphics[width=5cm]{Graphics/results/ShockTube/PertStudy/basedOn2DconstMass/Area2E}
}

\caption[Perturbation influence shock-tube with 1D constant mass particle distribution]{L--norm errors of $\rho, u, e$ for a series of simulation with increasing perturbation of initial particle position based on a 2D constant mass configuration with a support length of $l_s=0.025$ and a constant particle mass of $m=m=1.7361\cdot10{-5}$. Explanation on the meaning of the different areas is given in section (\ref{sec:2Dshock_simuSetup&Co}).}

\end{figure}


\chapter{Additional Poiseuille results}
