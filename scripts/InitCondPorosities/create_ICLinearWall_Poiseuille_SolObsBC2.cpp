...
int main () {
  ...
  const double domain_size_y=1.5;
  const int N_x=4;
  const int N_y=60;
  const int N=N_x*N_y;
  ...
  ofstream out("../../cases/LinearWall_Poiseuille_SolObsBC2.ivs");
 ...
 // output first line of .ivs file (geometrical data for wall)
  ...
  out.close();
  cout<<"<<LinearWall_Poiseuille_SolObsBC2.ivs>> is successfully written into <<sph-blitz/cases>>\n";
}
