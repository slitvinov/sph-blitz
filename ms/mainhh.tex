...
(?????heißt first order in dem fall, wert richtig ableitung nicht? oder heißt es first order convergence?? denn für thermal BC wurde z.b  rausgefunden, dass isotherme BC (implementiert mit const T) die Konvergenz von 2. auf first order reduziert...Cleary1999?????????) 


no--slip condition. A velocity assignment leading to a more accurate no--slip condition can be done by either mirroring or extrapolating the corresponding real particle velocity profile. \cite{Basa2009} tested these different approaches in a Poisseuille channel and found out that discrepancies with the exact solution mainly occurr right at the boundary during the initial phase.  According to these tests, mirroring of velocities is the least accurate of these more sophisticated methods, followed by linear and then quadratic extrapolation. 
...
\label{sec:TestSolobs_noSlip_BC}
...
\end{table}
Now there are clear differences in the accuracy of the different methods. By far the most inaccurate one is Method 2 with zero virtual ghost particle velocity followed by method 1 with only around a third of the error. Again around 40\% less in local error ($L_\infty$) is produced by method 3a, which now confirms that linear extrapolation of velocity is more accurate than mirroring \cite{Basa2009}. Another 40\% improvement is obtained by placing the first row of ghost particles directly on the wall surface.
For a supportlength $\geq2sx$ method 3b is therefore the method of choice for the modelling of a no--slip boundary condition. At least for a simple case as the poiseuille channel however, a supportlength of $2dx$ is sufficient to accurately represent the flow and even gives globally less error  (with the same number of particles) than the bigger supportlength simulation.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%following table put in comment
\begin{comment}
...
\hline
\hline
 &{\bf Method 1} & {\bf Method 2} & {\bf Method 3 a}& {\bf Method 3 a}\\
\hline
\hline
$L_1$&3.73746e-05 & 1.23141e-05 & 3.73765e-05& 1.23141e-05\\
\hline
$L_2$&0.000111057 & 2.08792e-05 & 0.000111063& 2.08792e-05\\
\hline
$L_\infty$&0.000463243 & 6.36603e-05 & 0.000463269& 6.36603e-05\\
\hline
\hline
\end{tabular}
...
Having in mind the test results for the analog(ous?) no--slip velocity boundary conditions, one expects that for a supportlength of $l_s=2dx$ no big difference in the results can be noticed (confirmed by table...). Therefore the simulations are run with $l_s=4dx$ here as well. 
...
As already observed for the no--slip velocity condition, one can see that for constant spacing (constant particle number) a bigger supportlength deteriorates generally the results (figure (\ref{})
A fist simulation is performed with $l_s=4dx$ and a spacing of $dx=0.025$ of which the Temperature profile is shown in figure (\ref{fig:SolObs_Thermal_BC1_profile}).
...
wenn ich als Resolution-mass Partikelzahl angebe ist doch das konvergenzverhalten was anderes als mit dx ($N~dx^2$), oder? was nimmt man da??? Cleary1999 sowie rest der Literatur nimmt Teilchenanzahl!
...
Visually one can hardly notice any difference in the temperature profiles for the two boundary condition methods.. Therefore it is not shown again at this place. However, the L-norm errors show a clear advantage for method 2 at this resolution (see table (\ref{tab:2DSPH_LinearWall_ThermalBC_Errors_supLen2.5dx})). A further variation of method 2 is tested where the first row of boundary particles is placed directly at the boundary surface (Method 2 b) and not as before at $dx/2$ away (Method 2 a). In the case of a supportlength of $2dx$ the latter is exactly the same as Method 1 as the particle on the boundary has exactly $T_w$ and the second row of ghost particles does just not have any influence on the real particles any more.


\begin{table}[h] % in eckigen Klammern steht die Platzierung, h steht f�r genau hier
\label{tab:2DSPH_LinearWall_ThermalBC_Errors_supLen2.5dx}
\centering
\begin{tabular}[c]{||l||r|r||} % die eckige Klammer steht f�r die Ausrichtung der Tabelle im Text: c steht f�r zentriert
\hline
\hline
 &{\bf Method 1} & {\bf Method 2} \\
\hline
\hline
$L_1$&0.00151539 &  0.00108916  \\
\hline
$L_2$&0.0032994 & 0.00220855  \\
\hline
$L_\infty$&0.0101702 & 0.00714104  \\
\hline
\hline
\end{tabular}


\caption[]{L-norm errors at the initial phase (at $t=1$) of the pure heat conduction problem with isothermal boundaries for the two boundary implementation methods at a spatial resolution of $dx=dy=0.025$ and a supportlength of $l_s=2dx$ }
\end{table}

A comparison of a set of simulationsn with $dx=dy=0.025$ and a supportlength of $l_s=2.5dx$ should show a difference 
...
\hline
\hline
 &{\bf Method 1} & {\bf Method 2 a} & {\bf Method 2 b}\\
\hline
\hline
$L_1$&0.00151539 &  0.00108916 & 0.00151539 \\
\hline
$L_2$&0.0032994 & 0.00220855 & 0.0032994 \\
\hline
$L_\infty$&0.0101702 & 0.00714104 & 0.0101702 \\
\hline
\hline
\end{tabular}
...
\end{table}
 ...
 The same resolution study as for method 1 has been conducted for method 2 to investigate possible differences in their convergence behaviour. In a first attempt the minimum admissible distance $d_a$ which is needed for the extrapolation of temperature according to equation (\ref{eq:IsothermalBC_T_extrapolation}) is evolved depending on x (see EQUATION...), as it was done for the equivalent no--slip velocity boundary implementation.
