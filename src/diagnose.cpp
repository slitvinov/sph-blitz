...
//							constructor
//----------------------------------------------------------------------------------------
Diagnose::Diagnose(const Initiation* const ini, Hydrodynamics &hydro)
{
	int k, l, m;
		...
		for (LlistNode<Node> *p = hydro.particle_list.first(); 
			...
			p = hydro.particle_list.next(p)) {
				...
				for(k = 0;  k < number_of_materials; k++) 
					if(strcmp(prtl->getMaterialName(), hydro.materials[k].material_name) == 0) 
						mtl_m[k] += prtl->getMass();
				ttl_m += prtl->getMass();
		}
...
void Diagnose::SaveStates(Hydrodynamics &hydro)
{
	...
	LlistNode<Node> *p = hydro.particle_list.first(); 
...
//----------------------------------------------------------------------------------------
void Diagnose::OutputProfile(double Time, const Initiation* const ini)
{
	int k, m;
	double Itime;
...
//----------------------------------------------------------------------------------------
void Diagnose::Average(ParticleManager &particles, MLS &mls, QuinticSpline &weight_function, const Initiation* const ini)
{
	int i, j, n;
	Vec2d pstn;
...
//----------------------------------------------------------------------------------------
void Diagnose::OutputAverage(double Time, const Initiation* const ini)
{
	int i, j;
	Vec2d pstn;
	double Itime;
...
//----------------------------------------------------------------------------------------
void Diagnose::KineticInformation(double Time, const Initiation* const ini, Hydrodynamics &hydro)
{
  int k;
  char file_name[50];
  ...
  for (LlistNode<Node> *p = hydro.particle_list.first(); 
    ...
    for(k = 0;  k < number_of_materials; k++) 
      if(strcmp(prtl->getMaterialName(), hydro.materials[k].material_name) == 0) {
	wght_cntr[k] += prtl->R()*prtl->getMass();
	wght_v[k] += prtl->U()*prtl->getMass();
      }
    glb_ave_Ek += 0.5*sqr(v_abs(prtl->U()))*prtl->getMass();
  }
  
  out<<ini->dms_time(Time)<<"  "<<ini->dms_mass(ttl_m)<<"  "
     <<ini->dms_energy(glb_ave_Ek)<<"  ";
  for(k = 0; k < number_of_materials; k++) {
    out<<ini->dms_length(wght_cntr[k][0]/mtl_m[k])<<"  "
       <<ini->dms_length(wght_cntr[k][1]/mtl_m[k])<<"  ";
    out<<ini->dms_length(wght_v[k][0]/mtl_m[k])<<"  "
       <<ini->dms_length(wght_v[k][1]/mtl_m[k])<<"  ";
  }
  out<<"\n";
  out.close();  
}
