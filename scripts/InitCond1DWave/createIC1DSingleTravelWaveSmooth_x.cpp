...
int main () {
  ...
  for(int i=0;i<N;i++) {
    if(x<lambda)    
      x_array[i]=x+(-A/c_s0*cos(-k*x-PI/2));// from dx=-A/c*cos(k(ct-x))
    else
      x_array[i]=x;
    x=x+dx;
    y_array[i]=0;
    if(x<lambda)
      vx_array[i]=k*A*sin(-k*x-PI/2);// from formula u=k*A*sin(k(ct-x))
    else
      vx_array[i]=0;
    vy_array[i]=0;
    if(x<lambda) 
      p_array[i]=p_0+k*c_s0*rho_0*A*sin(-k*x-PI/2);// from dp=p-p_0=k*c*rho_0*A*sin(k(ct-x))
    else  
      p_array[i]=p_0 ;
    if(x<lambda)
      rho_array[i]=rho_0+rho_0*k/c_s0*A*sin(-k*x-PI/2);// from s=(rho-rho_0)/rho_0=k/c*A*sin(k(ct-x))
    else
      rho_array[i]=rho_0;
  };
