...
int main (int argc, char *argv[]) {
  ...
  const double dy=dx;
  cout<<"dx=dy="<<dx<<endl;
  const double dx_hex=1.07*sqrt(2/sqrt(3))*dx;

  // m is initialized to zero, will be calculated in simulation
  // -->NO, due to discontinuity in lattice structure arond cylinder surface,
  ...
  // particle mass is = rho*dx*dy=rho*dx^2=4e-4
  // for pseudo hexagonal lattice the same (as we want equal mass particles)
 
  const double m=4e-4;
  
  // margi between cyliner surface and closest real particle placement
  ...
  // to make it eayier to use variable radii
  ...
  for(int i=0;i<N_cyl;i++) {
    ...
    for(double current_rad=All_Cyl_rad[i];current_rad>All_Cyl_rad[i]-2*supportlength;current_rad-=sqrt(3)/2*dx_hex) {
  ...
  for(double x=dx/2;x<domain_size_x;x+=dx) {
  ...
  ofstream out("../../cases/Porosities_SqareLatticeRealPrtl_CircularCyl.ivs");
  ...
  out.close();
  cout<<"<<Porosities_SqareLatticeRealPrtl_CircularCyl.ivs>> is successfully written into <<sph-blitz/cases>>\n";
}
