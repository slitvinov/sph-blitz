...
int main () {
  ...
  // in the first part the particle ensembles with same x coordinate
  // are formed and their IDs are stored

  // reading the sorted initial file sorted00000000.dat from the refSolution folder
  // create and open inputfile
  ifstream fin("../../results/refSolution/sorted00000000.dat", ios::in);
  // test if inputfile opended
  if (!fin) {
    cout<<"\n Cannot open refSolution/sorted00000000.dat \n" ;
    exit(1);
  }
  else cout<<"\n reading refSolution/sorted00000000.dat and forming particle emsembles for later averaging \n"; 
