...
//								constructor
//----------------------------------------------------------------------------------------
MLS::MLS(const Initiation* const  ini)
{
	int k;
...
//----------------------------------------------------------------------------------------
void MLS::MLSMapping(Vec2d &point, Llist<Node> &NNP_list, QuinticSpline &weight_function, int order)
{
		N = 0; //numbers of reference points
		p[0] = 1.0; p[1] = point[0]; p[2] = point[1];
		
		//iterate this cell list
		for (LlistNode<Node> *pstn = NNP_list.first(); 
			!NNP_list.isEnd(pstn); 
			pstn = NNP_list.next(pstn)) {
				
				//get particle data
				Node *prtl = NNP_list.retrieve(pstn);
