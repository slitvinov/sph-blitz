...
transformations to the original differential equations. %transformations necessary to ensure special properties, e.\ g.\ for momentum equation, simple application of SPH derivative approximation does not conserve momentum->either Lagrangian or transformation lead to forms that conserve momentum...
Depending on the
...
% \sProofOn
  ...
  \while[10]{\(while\ current\ time\ step\ \leq \ number\ of\ total\ time\ steps\)}
    ...
    \change
      ...
      \assign[10]{\(claculate\ new\ x(+1.0dt)\) \linebreak\((advance\ particles\ 1\ time step)\)}
    \ifend
